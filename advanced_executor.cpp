...
ExecutionResult AdvancedExecutor::executePowerShellScript(const std::vector<std::string> &commands)
{
    ...
    try
    {
        ...
        // ::executeScript was called without throwing an exception *within this function's scope*.        // Execute script using system call since executeScript function is not available
        ...
        std::string script_command = "powershell.exe -Command \"" + script + "\"";
        int exit_code = std::system(script_command.c_str());
        ...
        {
            result.error_message = "Script execution failed with exit code: " + std::to_string(exit_code);
        }
...
ExecutionResult AdvancedExecutor::executeVisionTask(const json &task_data)
{
    ...
    try
    {
        ...
        if (!result.success)
        {
            ...
            { // Fallback if final_result was empty
...
ExecutionResult AdvancedExecutor::executeUIAutomation(const json &automation_data)
{
    ...
    try
    {
        if (!automation_data.contains("actions"))
        {
            result.success = false;
